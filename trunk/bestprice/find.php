...
if(isset($_REQUEST['q'])){
	...
	if(isset($_REQUEST['site'])){
		...
		$data1 = $searchModel->read(null,array('category'=>$cat,'subcat'=>$subcat,'query'=>$query2,'is_followed'=>-1));
		if(sizeof($data1) > 0){ 
			$query_id = $data1[0]['query_id'];
			$searchModel->query('update `search` set `hits` = `hits` + 1,created_at = '.time().' where id = "'.$data1[0]['id'].'"');
		}else{
	...
	$return = array('untrusted'=>$untrusted,'query_id'=>$query_id,'ajax_parse'=>$ajaxParseSite,'data'=>$data,'result_time'=>date('d/m/y h:i a',$max),'result_number_time'=>$max,'error_sites'=>$errorSites,'empty_sites'=>$emptySites);
