...
function findPrice(site, cache, trust, changeSubCat, searchThis) {
	...
	if (site && site.length > 0) {
		var url = $('#ajax_url').val().replace('site', site) + 'find.php?q=' + query + '&cat=' + category + "&site=" + site + "&cache=" + cache + "&subcat=" + subcat + '&query_id=' + $('#query_id').val() + "&callback=?"+$('#session_name').val() + '=' + $('#session_id').val();
	} else {
		...
		$('#results').children('.website').each(function() {
			if ($(this).hasClass('website_error')) {
				...
				if (has_product == 'true') {
					...
					var url = $('#ajax_url').val().replace('site', website) + 'product.php?url=' + encodeURI(url) + '&website=' + encodeURI(website) + '&price=' + encodeURI(price) + "&stock=" + encodeURI(stock) + "&callback=?" +$('#session_name').val() + '=' + $('#session_id').val();
					var ajax = $.getJSON(url, function(data) {
